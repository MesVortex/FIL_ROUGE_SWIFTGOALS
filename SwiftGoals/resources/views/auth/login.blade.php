<x-authentication-layout>
  <section class="w-screen">
    <form id="form" class=" bg-transparent backdrop-filter backdrop-blur-sm bg-[#5882C1] bg-opacity-25 mt-10 pt-10 px-16 border-2 border-[#5882C1] border-opacity-15 rounded-3xl max-w-lg mx-auto" action="{{ route('user.login') }}" method="post">
      @csrf
      @method('POST')
